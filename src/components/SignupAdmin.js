...
import { Form, Input, Button, Avatar, Modal, message } from 'antd'; // Importez la composante message
    ...
    if (editor && avatar) {
      ...
      canvas.toBlob(async (blob) => {
        ...
        try {
          ...
          if (response.ok) {
            message.success('User created successfully'); // Affichez un message de succès
          } else {
  ...
  return (
    <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '100vh', backgroundColor: '#e6f7ff' }}>
      <div style={{ width: '400px', padding: '20px', border: '2px solid #ccc', borderRadius: '10px', backgroundColor: '#f9f9f9', boxShadow: '0 0 10px rgba(0, 0, 0, 0.1)' }}>
        <h2 style={{ marginBottom: '20px', textAlign: 'center' }}>Ajouter Administrateur</h2>
