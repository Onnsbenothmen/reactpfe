...
const SignupDirector = () => {
  ...
  useEffect(() => {
    // Assuming the current user ID is available as `currentUserId`
    const currentUserId = 1; // Replace with actual user ID retrieval logic
    axios.get(`http://localhost:5000/user/${currentUserId}/inst`)
      .then(response => {
        setInstanceId(response.data.nom_instance);
      })
      .catch(error => {
  ...
  const handleSubmit = async (values) => {
    try {
      ...
      data.append('instance_id', instanceId); // Include instance ID
      if (formData.avatar) {
        data.append('avatar', formData.avatar);
      }

      const response = await axios.post(`http://localhost:5000/registerAdmin/${newUserId}`, data, {
        headers: {
  ...
  return (
    ...
    >
      <div style={{ maxWidth: '500px', width: '100%', padding: '20px', backgroundColor: 'rgba(255, 255, 255, 0.8)', borderRadius: '8px', boxShadow: '0 0 10px rgba(0, 0, 0, 0.1)' }}>
        ...
        {!isRegistered ? (
          <Form onFinish={handleSubmit} initialValues={formData} layout="vertical">
